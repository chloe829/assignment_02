...
def clean_sales_data(raw_data: list[dict]) -> list[dict]:
    ...
    for row in raw_data:
        cleaned_currency = clean_currency(row["price"])
        cleaned_quantity = clean_quantity(row["qty"])
        cleaned_row = {
            "date": row["date"],
            "item": row["item"],
            "price": cleaned_currency,
            "qty": cleaned_quantity,
        }
      
        cleaned_row["total_revenue"] = cleaned_currency * cleaned_quantity
